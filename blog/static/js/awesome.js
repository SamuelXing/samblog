...
function showConfirm(title, text, fn_ok, fn_cancel) {
    var s = '<div id="div-confirm" class="modal">' +
...
//             		'<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>'+
            		...
            		'<button class="btn btn-primary x-ok"><i class="uk-icon-check"></i> 是</button>' +
            		'&nbsp;&nbsp;&nbsp;' +
            		'<button class="btn x-cancel"><i class="uk-icon-times"></i> 否</button>' +
            '</p></div></div></div>';
    $('body').append(s);
    var m = $('#div-confirm');
//     var modal = new $.fn.modal.Modal('#div-confirm');
    m.find('.modal-title').text(title);
    m.find('.modal-text').text(text);
